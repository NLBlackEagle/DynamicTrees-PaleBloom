...
public class ForgeConfigHandler {
    ...
    public static class PaleOakConfig {
        ...
        @Config.Comment("Chance (per tree) for a Pale Oak generated by the underground tree spawner (\"Enable Underground Growth\") to generate a Creaking Heart - separate from \"Creaking Heart Chance (Worldgen)\", which only applies to surface trees. Higher than the surface default by design: underground Resin Bulbs can't reach the daytime flora they'd normally charge from, so more underground Creaking Hearts means more chances for them to charge via the nightly active-heart search instead.")
        @Config.Name("Creaking Heart Chance (Underground)")
        @Config.RangeDouble(min = 0.0, max = 1.0)
        public double creakingHeartUndergroundChance = 0.35;
    ...
    public static class BloomingPaleOakConfig {
        ...
        @Config.Comment("Chance (per tree) for a Blooming Pale Oak generated by the underground tree spawner (\"Enable Underground Growth\") to generate a Creaking Heart - separate from \"Creaking Heart Chance (Worldgen)\", which only applies to surface trees. Higher than the surface default by design: underground Resin Bulbs can't reach the daytime flora they'd normally charge from, so more underground Creaking Hearts means more chances for them to charge via the nightly active-heart search instead.")
        @Config.Name("Creaking Heart Chance (Underground)")
        @Config.RangeDouble(min = 0.0, max = 1.0)
        public double creakingHeartUndergroundChance = 0.35;
    ...
    @Config.Comment("Cave Flora Options - flora moved over from OTG's own Plant(...) resources onto this addon's own worldgen pass, in the Pale Garden.")
    @Config.Name("Cave Flora Options")
    public static final CaveFloraConfig caveFlora = new CaveFloraConfig();

    public static class CaveFloraConfig {

        @Config.Comment("Flora scattered on exposed Grass or Pale Moss ground (true surface and any exposed cave floor within range alike) - one single top-to-bottom column scan per attempt, not a random 3D search, so each entry's cost scales predictably with its own frequency. Format: \"modid:block[:meta],frequency,rarity,minY,maxY,groundTypes\" - frequency is attempts per chunk, rarity is a 0-100 percent chance per successful attempt (matching the original OTG Plant(...) scale), groundTypes is \"GRASS\", \"MOSS\", or \"GRASS|MOSS\".")
        @Config.Name("Cave Flora Pool")
        public String[] caveFloraPool = {
                "palebloom:pale_plant_double,2,100.0,0,80,GRASS",
                "palebloom:pale_plant_double:1,2,100.0,0,80,GRASS",
                "palebloom:eyeblossom_closed,2,100.0,0,80,GRASS",
                "palebloom:bramble,2,100.0,0,70,GRASS",
                "palebloom:resin_bulb,2,100.0,0,60,GRASS|MOSS",
                "palebloom:incense_thorns,2,100.0,0,60,GRASS|MOSS",
                "palebloom:pollenhead,2,20.0,0,60,GRASS|MOSS"
        };

        @Config.Comment("Chance for a successfully-placed Incense Thorns from the pool above to also get a Resin Bulb placed directly beside it, so it actually has something in range to draw resin from. Independent of - and on top of - the pool's own separate Resin Bulb entry.")
        @Config.Name("Resin Bulb Near Incense Thorns Chance")
        @Config.RangeDouble(min = 0.0, max = 1.0)
        public double resinBulbNearIncenseThornsChance = 0.75;

        @Config.Comment("Attempts per chunk to replace exposed cave-floor ground (stone, dirt, or grass - anything not already Pale Moss) with Pale Moss Block. Same single-column-scan technique as the flora pool above; this replaces the ground block itself rather than placing something on top of it.")
        @Config.Name("Cave Moss Ground Attempts Per Chunk")
        @Config.RangeInt(min = 0, max = 64)
        public int caveMossGroundAttemptsPerChunk = 6;

        @Config.Comment("Percent chance (0-100, matching the flora pool's rarity scale) for each cave moss ground attempt to actually place.")
        @Config.Name("Cave Moss Ground Rarity")
        @Config.RangeDouble(min = 0.0, max = 100.0)
        public double caveMossGroundRarity = 100.0;

        @Config.Comment("Minimum Y level cave moss ground can replace.")
        @Config.Name("Cave Moss Ground Min Y")
        @Config.RangeInt(min = 0, max = 255)
        public int caveMossGroundMinY = 0;

        @Config.Comment("Maximum Y level cave moss ground can replace.")
        @Config.Name("Cave Moss Ground Max Y")
        @Config.RangeInt(min = 0, max = 255)
        public int caveMossGroundMaxY = 60;
    }
    ...
    public static class PaleLungConfig {
        ...
        @Config.Comment("Pollenhead and Incense Thorns only run their poison/Pale Lung aura when \"awake\", which Pale Bloom checks purely by world time (WorldPaleGarden.isNight) - light level and sky visibility never factor in, so one planted underground stays dormant all day regardless of how dark it actually is down there. Enabling this also treats any position with no sky light (fully underground, checked via canSeeSky) as night for that check, so these plants stay active during the day too as long as they're underground.")
        @Config.Name("Awaken Underground During Day")
        public boolean awakenUndergroundDuringDay = false;
    ...
    public static class MiscellaneousConfig {
        ...
        @Config.Comment("Multiplier on the amount of resin a Resin Bulb gains from a successful nightly active-Creaking-Heart charge (Pale Bloom's own base amount, scaled by this). 1.0 = unchanged. Doesn't affect the daytime flora-harvest amount, the search radius, or the check interval - just how much a hit is worth.")
        @Config.Name("Creaking Heart Resin Charge Multiplier")
        @Config.RangeDouble(min = 0.1, max = 10.0)
        public double creakingHeartResinChargeMultiplier = 3.0;
