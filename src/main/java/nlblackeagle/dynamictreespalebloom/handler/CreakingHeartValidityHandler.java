package nlblackeagle.dynamictreespalebloom.handler;
...
public class CreakingHeartValidityHandler {
    ...
    public void onCreakingUpdate(LivingEvent.LivingUpdateEvent event) {
        ...
        if (world.getTotalWorldTime() % 20L != 0L) return; // once a second is plenty

        BlockPos heartPos = creaking.getHeartPos();
        Block block = world.getBlockState(heartPos).getBlock();

        // Works for a real Creaking Heart or our tree-embedded variant, no matter
        // how the block actually got removed (mining, felling, explosion, etc.)
